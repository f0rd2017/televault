...
from televault.core.utils import sanitize_filename, sha256_file
...
ZIP_MT_ENV_PATH = "TELEVAULT_7Z_PATH"
ZIP_MT_ENV_THREADS = "TELEVAULT_7Z_THREADS"
...
def compress_file_to_temp_zip(
    ...
    temp_file = tempfile.NamedTemporaryFile(
        prefix="televault_zip_", suffix=".zip", delete=False
    )
...
def _compress_file_to_temp_zip_mt_7z(
    ...
    temp_file = tempfile.NamedTemporaryFile(
        prefix="televault_zip_mt_", suffix=".zip", delete=False
    )
    temp_path = Path(temp_file.name)
    temp_file.close()
