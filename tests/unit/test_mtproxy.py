...
@pytest.mark.parametrize(
    ...
    [
        "",
        "mtproto://justhost",
        f"mtproto://h:99999:{SECRET}",  # порт вне диапазона
        "tg://proxy?server=h&port=443",  # нет секрета
        "h:443",  # короткая форма без секрета
    ],
)
...
def test_build_telethon_proxy_socks_unchanged():
    # socks остаётся python_socks-кортежем (ведущий тип-строка, без 'mtproxy').
...
def test_proxy_for_set_proxy_strips_marker():
    # MTProto 4-кортеж → 3-кортеж (host, port, secret) для client.set_proxy.
    assert proxy_for_set_proxy(("mtproxy", "h", 443, SECRET)) == ("h", 443, SECRET)
    # socks/None — без изменений.
...
# ── Разрешение в цепочке (проба замокана) ───────────────────────────────────
